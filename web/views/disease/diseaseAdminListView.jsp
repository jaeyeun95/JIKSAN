...
<br>
   <form align="center" action="/jiksan/dsearch" method="post">
      <select name="category">
      		<option value="diseaseType" selected>질환 및 질병</option>
      		<option value="treatmentType">검사 및 치료</option>
      </select>
...
<% for(Disease d : list){ %>
		...
		<%}else {%>
			<button type="button" onclick="javascript:location.href='/jiksan/disease?disease_no=<%d.getDiseaseNo%>'" 
			style="width: 200px; height: 150px;"><%= d.getDiseaseNm() %></button>
			<% } %>
<% } //for each %>
<!-- </table> -->
...
&nbsp;
<% } %>
  새로운 건강 정보 (질환, 질병, 검사, 치료) <button onclick="javascript:location.href='/jiksan/views/disease/diseaseAdminWrite.jsp'">등록하기</button>
</div>
